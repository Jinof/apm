...
2. 从顶部 App Bar 打开独立的“模型设置”页面。默认 `http://127.0.0.1:11434` 指手机本身；使用 Mac mini VLM 时填写私网地址并勾选照片发送授权；
3. 从顶部人脸按钮打开“本地身份识别”，选择“人物”或“宠物”，再选择参考照片、点选一个检测框并命名。人物使用 YuNet + SFace；猫狗使用 SSD MobileNet V1 + MobileNetV3。检测、裁剪和 embedding 均由不带网络上报的 LiteRT Interpreter 在手机本地运行，同名可增加不同角度或光照的参考模板；
...
7. 授权照片后，照片墙会立即显示全部当前可访问照片，不要求先标注。可在“缩略图”和“明细”间切换；月历热力图显示每日照片数，点选有照片的日期可只看当天，再点“显示全部日期”清除筛选。点任一照片会在 APM 的只读全屏查看器中按原始宽高完整解码，缩放上限按图片和屏幕动态计算，至少为 5 倍且足以达到原始像素 1:1；受限拖动和相邻照片导航均不依赖外部看图应用；
